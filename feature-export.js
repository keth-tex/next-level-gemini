// === (Turndown-Setup und Scrape-Funktion) ===
...
turndownService.addRule('geminiCodeBlock', {
  ...
  replacement: function (content, node) {
    ...
    const metadata = JSON.stringify({ shorthand: shorthand, label: label });
...
function scrapeAndSendConversation(event) {
  ...
  console.log("Gemini LaTeX Export gestartet...");
  const conversation = [];
  ...
  if (blocks.length === 0) {
      console.error("Fehler: Konnte keine Konversations-Blöcke finden (z.B. '.conversation-container'). Selektoren prüfen.");
      return;
  }
  ...
  if (conversation.length > 0) {
    ...
    console.log(`Exportiere ${conversation.length} Prompt/Antwort-Paare...`);
  } else {
    console.error("Konnte keine gültigen Prompt/Antwort-Paare zum Exportieren finden.");
  }
}
