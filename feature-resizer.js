...
function autoResizeSidebar(e) {
  ...
  if (!sidebarEl) return;
// ... (Restlicher Code für autoResizeSidebar) ...
  const titles = sidebarEl.querySelectorAll('.conversation-title');
  if (titles.length === 0) {
      console.warn("Gemini Exporter: Konnte keine Chat-Titel (.conversation-title) für Auto-Resize finden.");
      return;
  }
